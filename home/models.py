...
class UserProfile(models.Model):
    image = models.ImageField(upload_to='profile/', verbose_name="Фотография", help_text="Фото должно быть Х на Х", blank=True, null=True)
...
class Transaction(models.Model):
    sender = models.ForeignKey(UserProfile, on_delete=models.PROTECT, related_name="sent_transactions", verbose_name="Отправитель")
    recipient = models.ForeignKey(UserProfile, on_delete=models.PROTECT, related_name="recipient_transactions", verbose_name="Получатель")
    ...
    def save(self, *args, **kwargs):
        ...
        self.sender.save()
        self.recipient.save()
        super().save(*args, **kwargs) # super обращается к главному save; *args, **kwargs - это случайные данные, которые передаются в функцию
    ...
    class Meta:
        ...
        ordering = ["-created_at"]               # для сортировки по убыванию в админке
